...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        kernel = np.tile(np.reshape(kernel, (1, 1, ksize, ksize)),(self.n_channels, 1, 1, 1))
    ...
    def forward(
        ...
        sum_image = low_frequencies + high_frequencies
        hybrid_image = torch.clamp(sum_image, 0, 1)
